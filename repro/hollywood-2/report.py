...
print 'Report on Hollywood-2 classification task'
print ''
...
print '  Average HOG fps: %.2f' % np.mean([y['Frame count'] / float(y['Decoding (sec)'] + y['Interp (sec)']['HOG'] + y['IntHist (sec)']['HOG'] + y['Desc (sec)']['HOG']) for y in features])
...
for k in sorted(set(classification) - set(['mAP'])):
	print '  %-15s\tAP = %.4f' % (k, classification[k])
print ''
print '  mean: %.4f' % classification['mAP']
